...
def extract_poses(video_path: str, conf: float = CONF_THRESHOLD, frame_skip: int = 1,
    ...
    # [refframe-engine] ADDITIVE DEVIATION: `progress_cb` kwarg (default None) is the
    # only change vs the source pose_extraction.py. When provided it is called
    # periodically from the pass-1 frame loop; None reproduces source behaviour exactly.
    """
    ...
    for result in model(
        ...
        raw_frames.append({"frame_idx": frame_idx,
                            ...
                            "dets":      dets})
        # [refframe-engine] ADDITIVE DEVIATION: report pass-1 progress every 25 frames.
        if progress_cb is not None and len(raw_frames) % 25 == 0:
            progress_cb(len(raw_frames), total)
