...
def refine_poses(video_path: str, pose_data: dict, mode: str = "balanced",
                 device: str = "cpu", max_frames: int | None = None,
                 progress_cb=None) -> dict:
    # [refframe-engine] ADDITIVE DEVIATION: `progress_cb` kwarg (default None) is the
    # only change vs the source pose_refine.py. When provided it is called from the
    # existing every-200-frames progress block; None reproduces source behaviour exactly.
    ...
    while frame_idx < last_todo:
        ...
        if len(refined_frames) % 200 == 0:
            ...
            # [refframe-engine] ADDITIVE DEVIATION: report refine progress.
            if progress_cb is not None:
                progress_cb(len(refined_frames), len(todo))
